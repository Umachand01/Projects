import tkinter  #tk-interface (graphica user interface library)
...
def set_tile(row,column):
 ...
 if curr_player==playero:  #swith player
    curr_player=playerx
 else:
    curr_player=playero
...
def check_winner():
    ...
    for column in range(3):
       if (board[0][column]["text"]==board[1][column]["text"]==board[2][column]["text"]
          ...
          game_over=True
          return
    #diagonally,check 3 word meet each
    if (board[0][0]["text"]==board[1][1]["text"]==board[2][2]["text"]
       ...
       game_over=True
       return   
    #anti-diagonally,check 3 word meet each 
...
label=tkinter.Label(frame,text=curr_player+"'s turn",font=("consolas",20), background=color_blue,
            foreground="white")
...
for row in range(3):
    for column in range(3):
        board[row][column]=tkinter.Button(frame,text="",font=("consolas",50,"bold"),        # its board coloring,taking size
                                          background=color_gray,foreground=color_blue,width=4,height=1,
                                          command=lambda row=row,column=column: set_tile(row,column))

        board[row][column].grid(row=row+1,column=column)

button = tkinter.Button(frame,text="restart",font=("consolas", 25),background=color_gray,            #it restart butten setup
...
window_height=window.winfo_height()          #it just take meddle position of the board
